    ...
    def next(self):
        ...
        if not self.position:
            ...
            if direction == "long" and long_signal:
                self.log("GOLDEN CROSS - Opening long position")
                self.order = self.buy(size=size)
            elif direction == "short" and short_signal:
                self.log("DEATH CROSS - Opening short position")
                self.order = self.sell(size=size)
            elif direction == "both":
                if long_signal:
                    self.log("GOLDEN CROSS (both mode) - Opening long position")
                    self.order = self.buy(size=size)
                elif short_signal:
                    self.log("DEATH CROSS (both mode) - Opening short position")
                    self.order = self.sell(size=size)
        else:
            ...
            if self.position_direction == "long":
                ...
                if pnl_pct <= -self.p.stop_loss:
                    ...
                    self.order = self.sell(size=self.position.size)
                elif self.crossover[0] < 0:  # Death cross while long
                    self.log("DEATH CROSS - Closing long position")
                    self.order = self.sell(size=self.position.size)
                ...
                if pnl_pct <= -self.p.stop_loss:
                    ...
                    self.order = self.buy(size=abs(self.position.size))
                elif self.crossover[0] > 0:  # Golden cross while short
                    self.log("GOLDEN CROSS - Closing short position")
                    self.order = self.buy(size=abs(self.position.size))
